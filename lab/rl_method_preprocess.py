...
def preprocess_new_data(input_file):
    ...
    df['Bitrate_Downloading']=df['Bitrate_Downloading'].astype(int64)
    # 删除不需要的列
    df = df.drop(['BitrateLevel', 'Clientnum'], axis=1)

    df = df.rename(columns={'Buffer':'Buffer_Length','Time':'time'})
    ...
    df=utils.convert_records_to_secondly(df)
    df=df.rename(columns={'Time':'time'})
    df['time'] = (df['time']).astype(int64)
    df=utils.interpolate_data(df)
    df=df.rename(columns={'time':'Time'})
    df.drop(columns=['id'],inplace=True)
    df['Time'] = df['Time']*1000
    df['Bitrate_Downloading']=df['Bitrate_Downloading'].astype(int64)
    # for buffer: if less than 4,set to 2,if less than 3,set to 0
    df.loc[df['Buffer_Length'] < 4, 'Buffer_Length'] = 2
    df.loc[df['Buffer_Length'] < 3, 'Buffer_Length'] = 0
    # of more than 31, set th 30
    df.loc[df['Buffer_Length'] > 31, 'Buffer_Length'] = 30
    return df
...
def merge_wave_df(dash_df, holo_df, output_filename):
    ...
    merged_df['throughput']=merged_df['throughput'].astype(int64)
