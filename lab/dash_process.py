...
def holo_csv(input_file, output_file):
    # 读取CSV文件
    df = pd.read_csv(input_file)
    ...
    with open(input_file, 'r', encoding='utf8') as infile, open(output_file, 'w', newline='',
        ...
        for line in infile:
            ...
            match = re.match(
                ...
                line)
            # match = re.match(
            #     r'.*?Received message (\d+) \| Transfer time: ([-\d.]+) ms \| Received at: (\d+) \| Content is : (.*)',
            #     line)
...
# 测试函数
def do_preprocess(base_name,dir,holo_csv_file):
    # 控制台输出的，包含tcp状态的文件
    input_file = base_name + '.log'
    output_file = base_name + '_status.csv'
    log_convert_to_csv(dir + input_file, dir + output_file)
    # dash的比特率记录文件，插件抓的
    ...
    output_file = 'holo.csv'
    holo_df=holo_csv(dir + input_file, dir + output_file)
    merge_wave_df(dash_df, holo_df, dir + base_name + '_wave.csv')

# do_preprocess('bola1','fin2case/')
# do_preprocess()
